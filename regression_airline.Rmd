...
```{r}
df<- read_xlsx('database2010_2019_distance2.xlsx') %>%
  select(AIRCRAFT_TYPE, ln_AIR_FUELS_ISSUED,
         ln_DEPARTURES_SCHEDULED, ln_AIR_TIME, ln_distance,
        time, AIRLINE_ID) %>%
  mutate(AIRCRAFT_TYPE=recode(AIRCRAFT_TYPE, '612'='B737-700', '614'='B737-800','694'='A320','721'='A321', '634'='B737-900','698'='A319','622'='B757-200', '627'='B777','637'='B777', '359'='A350'))
...
```{r}
# Recode column names
colnames(aggregated_data)<- c('aircraft_type', 'ln_fuel', 'ln_number_of_flights', 'ln_airborne_hours', 'time','ln_distance' ,'airline_id', 'ln_average_fuel_price', 'ratio')
```
...
formula5<- ln_fuel ~ ln_number_of_flights + ln_airborne_hours  + as.factor(aircraft_type) * ln_average_fuel_price + as.factor(name) * ln_average_fuel_price
m5<- lm(formula = formula5, data=aggregated_data)
summary(m5)

formula6<- ln_fuel ~ ln_number_of_flights + ln_airborne_hours
